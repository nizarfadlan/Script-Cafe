...
const MenuItemComponent = ({
  ...
  const { query } = router;
  const [pageItem, setPageItem] = useState<number>(query.pageItem ? parseInt(query.pageItem as string) : 0);

  const { data, fetchNextPage, isLoading, isFetchingNextPage, isError } = api.item.getAll.useInfiniteQuery(
    {
      limit,
      search,
      status: "available",
  ...
  const handleFetchNextPage = useCallback(async() => {
    ...
    router.query.pageItem = `${pageItem + 1}`;
    await router.push(router, undefined, { scroll: false });
  }, [fetchNextPage, router, pageItem]);

  const { pages: pageData = [] } = data || {};
  const showData: MenuItem[] = [];
  if (pageItem > 0 && pageData.length > 0) {
    pageData.map((page) => {
      showData.push(...page.items);
    });
  }  else {
    showData.push(...pageData[0]?.items ?? []);
  }

  const nextCursor = pageData[pageItem]?.nextCursor;

  useEffect(() => {
    const fetchPage = async() => {
      if (query.pageItem) {
        setPageItem(parseInt(query.pageItem as string));
        await fetchNextPage();
      }
    }
